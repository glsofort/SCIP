...
while (<file1>){
 ...
 unless (-e "$dir/app_temp_file/$split1[0]/$split1[0].$split1[1].$split1[2].$split1[3].$split1[4].log.txt.gz"){
  ...
  system ("perl $dir/SCIP_pri_03_hg19.pl -c $split1[1] -s $split1[2] -e $split1[3] -p $split1[0] -t $split1[4] -@ 1");
  system ("perl $dir/SCIP_pri_04_hg19.pl -c $split1[1] -s $split1[2] -e $split1[3] -p $split1[0] -t $split1[4] -@ 1");
  ...
  system ("perl $dir/SCIP_pri_08_hg19.pl -c $split1[1] -s $split1[2] -e $split1[3] -p $split1[0] -t $split1[4]");
  my $end=localtime();
  my $end_time=time();
  ...
  open file2, "<$dir/pipeline_config.txt";
  while (<file2>){
   chomp;
   my @split2=split /\t/,$_;
